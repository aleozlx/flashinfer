...
LAYERS = [2, 8]
SIZES = {1536: "BUGGY (round_up(1440, 256))", 2048: "FIXED (round_up(1440, 1024))"}

# Known model dimensions for gpt-oss-120b (used by --random-weights mode)
# w13b (gate_up_proj_blocks) shape: (E, 2*FULL_I, H/SF_VEC, SF_VEC/2) uint8
#   → reshape(E, 2*FULL_I, -1) gives (E, 2*FULL_I, H/2) packed FP4 bytes
# w13s (gate_up_proj_scales) shape: (E, 2*FULL_I, H/SF_VEC) — one scale per block
# w2b/w2s follow the same pattern for the down projection (H, FULL_I)
E_TOTAL_MODEL = 128
FULL_I_MODEL = 2880  # intermediate size before TP
H_RAW_MODEL = 2880  # hidden size before padding (→ 3072 after pad to 256)
...
def shuffle_expert(w, ws, b):
    ...
    return (
        shuffle_matrix_a(w.view(torch.uint8), TILE_M),
...
def generate_random_weights():
    """Generate random FP4-format tensors matching gpt-oss-120b shapes."""
    E, I, H = E_TOTAL_MODEL, FULL_I_MODEL, H_RAW_MODEL
    nsf_h = H // SF_VEC  # 90: scale-factor groups along hidden dim
    nsf_i = I // SF_VEC  # 90: scale-factor groups along intermediate dim
    bpsf = SF_VEC // 2  # 16: packed FP4 bytes per scale-factor group

    # FP4 packed blocks: 4D so shape[2]*SF_VEC recovers H
    w13b = torch.randint(0, 256, (E, 2 * I, nsf_h, bpsf), dtype=torch.uint8)
    w2b = torch.randint(0, 256, (E, H, nsf_i, bpsf), dtype=torch.uint8)
    # Block scales: one per SF_VEC elements (avoid zero to prevent div-by-zero)
    w13s = torch.randint(1, 255, (E, 2 * I, nsf_h), dtype=torch.uint8)
    w2s = torch.randint(1, 255, (E, H, nsf_i), dtype=torch.uint8)
    # Biases (small random)
    w13bi = torch.randn(E, 2 * I) * 0.01
    w2bi = torch.randn(E, H) * 0.01
    return w13b, w13s, w13bi, w2b, w2s, w2bi


def load_layer(
    model_dir, layer_idx, inter_size, tp_rank=0, tp_size=2, random_weights=False
):
    """Load one MoE layer: TP-shard, pad, shuffle for the kernel."""
    t0 = time.time()

    if random_weights:
        w13b, w13s, w13bi, w2b, w2s, w2bi = generate_random_weights()
    else:
    ...
    return dict(
        ...
        w13s=torch.stack(w13s_sh)
        .reshape(n, 2 * P, pH // SF_VEC)
...
def run_test(w, inter_size, num_iters):
    ...
    for i in range(num_iters):
        ...
        out = trtllm_fp4_block_scale_moe(
            ...
            tune_max_num_tokens=max(n, 8),
        )[0]
        if out.isnan().any().item():
            nan_count += 1
            if nan_count <= 3:
                print(
                    f"    NaN at iter {i}, tokens={n}, "
...
def main():
    ...
    ap.add_argument(
        "--model-dir", default=os.environ.get("MODEL_DIR", "openai/gpt-oss-120b")
    )
    ap.add_argument("--num-iters", type=int, default=10000)
    ap.add_argument(
        "--random-weights",
        action="store_true",
        help="Use random tensors instead of real model weights (no model needed)",
    )
    args = ap.parse_args()
    ...
    gpu, drv, fi = env_info()
    weight_src = "RANDOM" if args.random_weights else model_dir
    ...
    for layer in LAYERS:
        for isize, desc in SIZES.items():
            ...
            w = load_layer(model_dir, layer, isize, random_weights=args.random_weights)
            t0 = time.time()
            nans = run_test(w, isize, args.num_iters)
            tag = "FAIL" if nans > 0 else "PASS"
            print(
                f"  NaN={nans}/{args.num_iters} "
    ...
    for layer, isize, nans, total, tag in results:
        print(
            f"  Layer {layer}, I={isize}: NaN={nans}/{total} "
